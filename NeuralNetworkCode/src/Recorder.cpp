...
Recorder::Recorder(const std::shared_ptr<NeuronPopSample> neurons, const std::shared_ptr<SynapseSample> synapses,
  ...
  currentContrBin.resize((static_cast<size_t>(totalNeuronPops) + 1) *
                         static_cast<size_t>(std::reduce(currentContributionsToRecord.begin(), currentContributionsToRecord.end())));
...
void Recorder::SetFilenameDate() {
  ...
  Windows = true;
#endif
  // auto localTime = std::chrono::floor<std::chrono::seconds>(std::chrono::zoned_time{std::chrono::current_zone(),
  // std::chrono::system_clock::now()}.get_local_time()); //Does not work in GCC
...
void Recorder::WriteSteadyStates() const {
  ...
  for (PopInt popIndex = 0; popIndex < neurons->GetTotalPopulations(); popIndex++) {
    ...
    if (pop->HasSteadyState()) {
      ...
      for (auto &[prefix, value] : pop->GetSteadyStateData()) {
        ssStream << prefix << "\t\t\t" << std::to_string(value) << "\n";
      }
    }
  }
  ssStream.close();
...
void Recorder::SaveParameters(std::ofstream &wParameterStream) const {
  ...
  wParameterStream << "recorder_noNeuronsConnectivity\t\t" << std::to_string(noNeuronsConnectivity)
                   << "\t\t\t#saves connectivity matrices for the first x neurons of each populations\n";
  wParameterStream << "recorder_noNeuronsDelay\t\t\t" << std::to_string(noNeuronsDelay)
                   << "\t\t\t#saves delay connectivity matrices for the first x neurons of each populations\n";
  wParameterStream << "recorder_noNeuronsJPot\t\t\t" << std::to_string(noNeuronsJPot)
                   << "\t\t\t\t#saves Jpot connectivity matrices for the first x neurons of each populations\n";
  // Adv Recorder SP
  wParameterStream << "recorder_binSize\t\t\t" << std::to_string(GetAveragingSteps() * infoGlobal->dtTimestep)
                   << " #secs\t\t#Bin size over which data saved in main recording data file is average over\n";
  ...
  wParameterStream << "recorder_Heatmap\t\t\t" << std::to_string(recordedHeatmap)
                   << "\t\t\t#Number of bins used to represent each dimension of the spatial domain in the firing rates Heatmap\n";
...
void Recorder::LoadParameters(const std::vector<FileEntry> &recorderParameters) {
  ...
  if (std::accumulate(heteroSynTracker.begin(), heteroSynTracker.end(), 0,
                      [](int accumulator, std::pair<NeuronInt, signed long> pop) { return accumulator + (pop.first * pop.second); }) != 0) {
...
void Recorder::SetNoRasterplotNeurons(const std::vector<std::string> &parameterValues) {
  ...
  for (PopInt neuronPop : std::ranges::views::iota(0, std::min(totalNeuronPops, static_cast<PopInt>(parameterValues.size())))) {
    ...
    if ((noRasterPlotNeurons.at(neuronPop) > neurons->GetNeuronsPop(neuronPop)) || (noRasterPlotNeurons.at(neuronPop) < 0)) {
...
void Recorder::WriteDataHeaderAverages() {
  ...
  for (PopInt targetPop : std::ranges::views::iota(0, totalNeuronPops)) {
    for (PopInt sourcePop : std::ranges::views::iota(0, totalNeuronPops)) {
      this->fileStreams.averagesFileStream << "# " + std::to_string(columnTotal) + " mu_" << std::to_string(targetPop) << "_"
                                           << std::to_string(sourcePop) << "/tau_m (dmV/sec) \n";
      columnTotal++;
    }
  }
  for (PopInt neuronPop : std::ranges::views::iota(0, totalNeuronPops)) {
  ...
  for (PopInt neuronPop : std::ranges::views::iota(0, totalNeuronPops)) {
    this->fileStreams.averagesFileStream << "#" + std::to_string(columnTotal) + " Quenched fluctuations of mu_" << neuronPop
                                         << "/tau_m (dmV/sec) = sqrt(PopAver(TempAver(mu_i)^2) - PopAver(TempAver(mu_i))^2) \n";
    columnTotal++;
  }
...
void Recorder::WriteDataHeaderHeteroSynapses() {
  ...
  this->fileStreams.heteroSynFileStream << "#1 t (secs.)\t 2-"
                                        << 1 + std::accumulate(heteroSynTracker.begin(), heteroSynTracker.end(), 0,
                                                               [](int accumulator, std::pair<NeuronInt, signed long> pop) {
                                                                 return accumulator + (pop.first * pop.second);
                                                               })
                                        << " Profile_pop_id_neuron_id \n";
...
void Recorder::WriteDataHeaderHeteroSynapsesOverall() {
  ...
  this->fileStreams.hSOverallFileStream << "#1 t (secs.)\t 2-"
                                        << 1 + std::accumulate(heteroSynTracker.begin(), heteroSynTracker.end(), 0,
                                                               [](int accumulator, std::pair<NeuronInt, signed long> pop) {
                                                                 return accumulator + (pop.first * pop.second);
                                                               })
      ...
      for (PopInt targetPop : std::ranges::views::iota(0, totalNeuronPops)) {
        if (currentBin.noRecordedSynapses.at(targetPop).at(sourcePop) > 0) {
          for (double recordedValue : currentBin.synapticState.at(targetPop).at(sourcePop)) {
            SaveDoubleFile(this->fileStreams.synStatesFileStream,
  ...
  for (PopInt sourcePop : std::ranges::views::iota(0, totalNeuronPops)) {
    ...
    if (recordedHeatmap != 0) {
      if (infoGlobal->dimensions == 2) {
        ...
        //     currentBin.heatmap.at(sourcePop).at(static_cast<size_t>(recordedHeatmap) * Ybin + Xbin) += 1 /
        //     static_cast<double>(densimap.at(sourcePop).at(static_cast<size_t>(recordedHeatmap) * Ybin + Xbin));
        // }
      } else {
        std::for_each(PAR_UNSEQ, spikers.begin(), spikers.end(), [this, sourcePop](double spiker) {
          double Xbin = static_cast<int>(neurons->GetXPosition(sourcePop, spiker) * recordedHeatmap / infoGlobal->xAxisLength);
    ...
    for (NeuronInt spiker : spikers) {
      for (PopInt targetPop : std::ranges::views::iota(0, totalNeuronPops)) {
        ...
        // std::to_string(this->infoGlobal->timeStep) << "\n";
