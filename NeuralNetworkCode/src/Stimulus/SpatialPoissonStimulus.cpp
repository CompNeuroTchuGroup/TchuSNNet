...
SpatialPoissonStimulus::SpatialPoissonStimulus(std::shared_ptr<NeuronPopSample> neurons, std::vector<FileEntry>& stimulusParameters, GlobalSimInfo*  infoGlobal):Stimulus(neurons,infoGlobal){
...
void SpatialPoissonStimulus::LoadParameters(const std::vector<FileEntry>& stimulusParameters){
	...
	//With this we can assume the stimulusStep vector will be sorted according to time (reverse sorted before, now because of different finding algo, regular sorted)
	PostLoadParameters();
}
...
void SpatialPoissonStimulus::SaveParameters(std::ofstream& wParameterStream) const{
    ...
    wParameterStream <<  "#\t\t" << IDstringSpatialPoissonStimulus << ": noExternalNeurons external neurons with (poisson) firing rate defined in stimulus_step are connected with population X with strength J_X using a distance-dependant connectivity.\n";
}
...
void SpatialPoissonStimulus::PostLoadParameters() {
	...
	for(PopInt neuronPop : std::ranges::views::iota(0,neurons->GetTotalPopulations())){
		externalCurrents.at(neuronPop) = ((J_External.at(neuronPop) * pow(GetScaling(neuronPop),(infoGlobal->networkScaling_synStrength)))*infoGlobal->dtTimestep)/inputTau;
	}
	...
	poissonDistr=std::poisson_distribution<int>(infoGlobal->dtTimestep*static_cast<double>(noExternalNeurons)*stimulusSteps.front().parameterValues.at(0));
...
void SpatialPoissonStimulus::SetSignalMatrix() {
	...
	//For every neuron that fired this timestep: pick the neuron and distribute the current
		...
		for (PopInt neuronPop : std::ranges::views::iota(0,totalNeuronPops)) {
			// double current = ((J_External.at(neuronPop) * pow(GetScaling(neuronPop),(infoGlobal->networkScaling_synStrength)))*infoGlobal->dtTimestep)/inputTau;
			std::for_each(externalConnectivity.at(sourceNeuronExt).at(neuronPop).begin(), externalConnectivity.at(sourceNeuronExt).at(neuronPop).end(), [this,neuronPop](NeuronInt synapticTarget){
				...
				for (int iteratorX = -1;iteratorX <= 1;iteratorX++) {
					if (infoGlobal->dimensions == 1) {
						...
						for (int iteratorY = -1;iteratorY <= 1;iteratorY++) {
							distance = pow(sourcePosX - targetPosX + iteratorX * infoGlobal->xAxisLength, 2) + pow(sourcePosY - targetPosY + iteratorY * infoGlobal->yAxisLength, 2);
							probability += peakConnectProb.at(neuronPop) * exp(-distance / (2 * pow(lengthScale.at(neuronPop), 2)));
						}
					}
				}
...
double SpatialPoissonStimulus::GetScaling(PopInt neuronPop) const {
	...
	if (infoGlobal->networkScaling_mode == 0) {
		if (infoGlobal->dimensions == 1){
			avgConnExtPop = static_cast<double>(2 * (4 * atan(1)) * noExternalNeurons / (infoGlobal->xAxisLength) * peakConnectProb.at(neuronPop) * lengthScale.at(neuronPop));
		} else {
			avgConnExtPop = static_cast<double>(2 * (4 * atan(1)) * noExternalNeurons/(infoGlobal->xAxisLength*infoGlobal->yAxisLength) * peakConnectProb.at(neuronPop) * pow(lengthScale.at(neuronPop), 2));
		}
