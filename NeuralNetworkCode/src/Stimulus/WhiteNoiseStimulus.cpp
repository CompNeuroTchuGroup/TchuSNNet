...
WhiteNoiseStimulus::WhiteNoiseStimulus(std::shared_ptr<NeuronPopSample> neurons, std::vector<FileEntry> &stimulusParameters,
...
void WhiteNoiseStimulus::SaveParameters(std::ofstream &wParameterStream) const {
  ...
  for (const StepStruct &step : meanCurrent) {
    ...
    wParameterStream
        << " #[column 1: input for population 1, column 2: input for neuronPop. 2, ... , last column: time until which input is set. Dimensions: [mV/sec , secs.]\n";
    //*stream << " [mV/sec -- sec]\n";
  }
  ...
  for (const StepStruct &step : sigmaCurrent) {
    ...
    wParameterStream << std::to_string(static_cast<double>(step.endTimeStep) * infoGlobal->dtTimestep) << " \t";
    wParameterStream
        << " #[column 1: input for population 1, column 2: input for neuronPop. 2, ... , last column: time until which input is set. Dimensions: [mV/sqrt(sec) , secs.]\n";
  }
...
void WhiteNoiseStimulus::RecalculateMeansCache(StepStruct *meanStep) {
  ...
  for (PopInt neuronPop : std::ranges::views::iota(0, neurons->GetTotalPopulations())) {
    cachedMeans.at(neuronPop) =
        meanStep->parameterValues.at(neuronPop) * dtTimestep * pow(GetScaling(neuronPop), -(infoGlobal->networkScaling_synStrength));
  }
}
...
void WhiteNoiseStimulus::PostLoadParameters() {
  ...
  for (PopInt neuronPop : std::ranges::views::iota(0, neurons->GetTotalPopulations())) {
    cachedMeans.at(neuronPop) =
        currentMeanStep->parameterValues.at(neuronPop) * dtTimestep * pow(GetScaling(neuronPop), -(infoGlobal->networkScaling_synStrength));
  }
  for (PopInt neuronPop : std::ranges::views::iota(0, neurons->GetTotalPopulations())) {
