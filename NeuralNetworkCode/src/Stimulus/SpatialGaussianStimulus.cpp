...
SpatialGaussianStimulus::SpatialGaussianStimulus(std::shared_ptr<NeuronPopSample> neurons,std::vector<FileEntry>& stimulusParameters,GlobalSimInfo*  infoGlobal):Stimulus(neurons,infoGlobal){
...
void SpatialGaussianStimulus::SaveParameters(std::ofstream& wParameterStream) const{
	...
	for (int gaussianIndex : std::ranges::views::iota(0,noGaussians)){
		for (const StepStruct& step : maxCurrent.at(gaussianIndex)) {
			...
			wParameterStream << " #[column i: input to neurons of population i, at the center of the gaussian, last column: time until which input is set. Dimensions: [mV/sec , secs.]\n";
		}
		for (const StepStruct& step : sigmaCurrentT.at(gaussianIndex)) {
			...
			wParameterStream << std::to_string(static_cast<double>(step.endTimeStep)*infoGlobal->dtTimestep) << " \t";
			wParameterStream << " #[column i: relative input noise to population i (relative to the mean current), last column: time until which input is set. Dimensions: [ -  , secs.]\n";
		}
		...
		for (const StepStruct& step : sigmaCurrentX.at(gaussianIndex)) {
			...
			wParameterStream << std::to_string(static_cast<double>(step.endTimeStep)*infoGlobal->dtTimestep) << " \t";
			wParameterStream << " #[column 1: spatial spread (std of the Gaussian) of the input to all populations, last column: time until which input is set. Dimensions: [mm , secs.]\n";
		}
	}
	...
	wParameterStream <<  "#\t\tRI_{i,ext}/tauM*dt = meanCurrent_i*dt*exp(-d{i}^2/(2sigmaCurrent_x)) + sqrt(dt)*sigmaCurrent_t_i*NormalDistribution(0,1)\t Where d{i} is the distance of neuron i to the center of the domain\n";
}
void SpatialGaussianStimulus::PostLoadParameters() {
	...
	if (backgroundNoise.empty() || std::any_of(sigmaCurrentX.begin(), sigmaCurrentX.end(),stepMatrixEmptyBool)||std::any_of(sigmaCurrentT.begin(), sigmaCurrentT.end(),stepMatrixEmptyBool) || std::any_of(maxCurrent.begin(), maxCurrent.end(),stepMatrixEmptyBool)){
		wellDefined=false;
	...
	if (infoGlobal->dimensions==2){
		calculateDistance = [this](PopInt neuronPop, NeuronInt neuron, int gaussianIndex){return pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2) + pow(neurons->GetYPosition(neuronPop, neuron) - gaussianPositionY.at(gaussianIndex)*infoGlobal->yAxisLength, 2);};
	} else {
		calculateDistance = [this](PopInt neuronPop, NeuronInt neuron, int gaussianIndex){return pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2);};
	}
	...
	for (int gaussianIndex : std::ranges::views::iota(0, noGaussians)){
		...
		for (PopInt neuronPop : std::ranges::views::iota(0,neurons->GetTotalPopulations())){
			...
			for (NeuronInt neuron : std::ranges::views::iota(0,neurons->GetNeuronsPop(neuronPop))){
				// double distance= pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2) + pow(neurons->GetYPosition(neuronPop, neuron) - gaussianPositionY.at(gaussianIndex)*infoGlobal->yAxisLength, 2);
				cachedMuNeuronFactors.at(gaussianIndex).at(neuronPop).at(neuron) =scalingConstantDt*exp(-calculateDistance(neuronPop, neuron,gaussianIndex) / (2 * pow(sigmaX, 2)));
			}
		}
	}
	std::cout<<"Done.\n"<<std::endl;
...
void SpatialGaussianStimulus::RecalculateFactors(int gaussianIndex, double sigmaX) {
	...
	// if (infoGlobal->dimensions==2){
	// 	distanceCalc = [this](PopInt neuronPop, NeuronInt neuron, int gaussianIndex){return pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2) + pow(neurons->GetYPosition(neuronPop, neuron) - gaussianPositionY.at(gaussianIndex)*infoGlobal->yAxisLength, 2);};
	// } else {
	// 	distanceCalc = [this](PopInt neuronPop, NeuronInt neuron, int gaussianIndex){return pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2) + pow(neurons->GetYPosition(neuronPop, neuron) - gaussianPositionY.at(gaussianIndex)*infoGlobal->yAxisLength, 2);};
	// }
	for (PopInt neuronPop : std::ranges::views::iota(0,neurons->GetTotalPopulations())){
		...
		for (NeuronInt neuron : std::ranges::views::iota(0,neurons->GetNeuronsPop(neuronPop))){
			// double distance{};
			// if (infoGlobal->dimensions==2){
			// 	distance= pow(neurons->GetXPosition(neuronPop, neuron) - gaussianPositionX.at(gaussianIndex)*infoGlobal->xAxisLength, 2) + pow(neurons->GetYPosition(neuronPop, neuron) - gaussianPositionY.at(gaussianIndex)*infoGlobal->yAxisLength, 2);
...
void SpatialGaussianStimulus::SetSignalMatrix(){
	...
	for (int gaussianIndex : std::ranges::views::iota(0,noGaussians)) {
		...
		if((sigmaCurrentXPtrs.at(gaussianIndex) != &sigmaCurrentX.at(gaussianIndex).back()) && (sigmaCurrentXPtrs.at(gaussianIndex)->endTimeStep<=timeStep)){
		...
		if((sigmaCurrentTPtrs.at(gaussianIndex) != &sigmaCurrentT.at(gaussianIndex).back()) && (sigmaCurrentTPtrs.at(gaussianIndex)->endTimeStep<=timeStep)){
		...
		for (PopInt neuronPop : std::ranges::views::iota(0,totalNeuronPops)) {
			...
			for (NeuronInt neuron : std::ranges::views::iota(0,neurons->GetNeuronsPop(neuronPop))) {
				...
				//		d = pow(neurons->GetX_Pos(neuronPop, i) - infoGlobal->xAxisLength *(GPos_X[Gauss_index] + x_rep), 2) + pow(neurons->GetY_Pos(neuronPop, i) - infoGlobal->yAxisLength *(GPos_Y[Gauss_index] + y_rep), 2);
				//		if (infoGlobal->Dimensions == 2 || y_rep == 0)
				//			mu = mu + mu_max * dtTimestep*pow(s, infoGlobal->networkScaling_synStrength)*exp(-d / (2 * pow(sigmax, 2)));
				//	}
				//}
				if (gaussianIndex == 0){
					signalMatrix.at(neuronPop).at(neuron) = mu * (1 + sigmaT * standardDistribution(generator)) + backgroundNoisePtr->parameterValues.at(neuronPop) * dtSqrt * standardDistribution(generator);
				} else{
	...
	// 	accumulator=std::accumulate(signalVector.begin(), signalVector.end(), accumulator, [](double accumulator, double value){return accumulator+value;});
	// }
