...
WhiteNoiseLinear::WhiteNoiseLinear(std::shared_ptr<NeuronPopSample> neurons, std::vector<FileEntry>& stimulusParameters, GlobalSimInfo*  infoGlobal) :Stimulus(neurons, infoGlobal) {
	...
	for (auto& [parameterName, parameterValues] : stimulusParameters) {
		...
		if ((parameterName.find("meanCurrent") != std::string::npos)) {
			...
			for (PopInt readingIndex : std::ranges::views::iota(0, totalNeuronPops)){
				//Here we are only reading the input parameters, which they should be 2*(totalNeuronPops+1). Their order of usage will differ of order of reading
...
void WhiteNoiseLinear::SaveParameters(std::ofstream& wParameterStream) const {
	...
	for (const StepStruct & step : meanCurrent) {
		...
		wParameterStream << " #[col 1: input to pop0 at t_0, col 2: pop1 at t_0, ... colP+1: pop1 t_f, ... col2P: popN t_f, t0, tf. Dimensions: [mV/sec , secs.]\n";
	}
	...
	for (const StepStruct & step : sigmaCurrent) {
		...
		wParameterStream << std::to_string(static_cast<double>(step.startTimeStep)*infoGlobal->dtTimestep) << " \t";
		wParameterStream << std::to_string(static_cast<double>(step.endTimeStep)*infoGlobal->dtTimestep) << " \t";
		wParameterStream << " #[col 1: input to pop0 at t_0, col 2: pop1 at t_0, ... colP+1: pop1 t_f, ... col2P: popN t_f, t0, tf. Dimensions: [mV/sqrt(sec) , secs.]\n";
	}
...
void WhiteNoiseLinear::PostLoadParameters() {
	...
	for (PopInt neuronPop : std::ranges::views::iota(0,totalPopulations)){
		meanScalingCache.at(neuronPop) = (meanCurrentPtr->parameterValues.at(neuronPop+totalPopulations)-meanCurrentPtr->parameterValues.at(neuronPop))/(meanCurrentPtr->endTimeStep-meanCurrentPtr->startTimeStep);
	}
	for (PopInt neuronPop : std::ranges::views::iota(0,totalPopulations)){
		sigmaScalingCache.at(neuronPop) = (sigmaCurrentPtr->parameterValues.at(neuronPop+totalPopulations)-sigmaCurrentPtr->parameterValues.at(neuronPop))/(sigmaCurrentPtr->endTimeStep-sigmaCurrentPtr->startTimeStep);
	}
	std::cout<<"Done.\n"<<std::endl;
}
...
void WhiteNoiseLinear::SetSignalMatrix() {
	...
	for (PopInt neuronPop : std::ranges::views::iota(0,neurons->GetTotalPopulations())) {
		...
		for (NeuronInt neuron : std::ranges::views::iota (0, neurons->GetNeuronsPop(neuronPop))){
			...
			//this is constant and independent of the rest of the simulation. This could be precalculated but it would require a lot of memory (one matrix per timestep)
		}
	}
}
...
void WhiteNoiseLinear::RecalculateMeanScalingCache(StepStruct *meanStep) {
	PopInt totalPopulations {neurons->GetTotalPopulations()};
	for (PopInt neuronPop : std::ranges::views::iota(0,totalPopulations)){
		meanScalingCache.at(neuronPop) = (meanStep->parameterValues.at(neuronPop+totalPopulations)-meanStep->parameterValues.at(neuronPop))/(meanStep->endTimeStep-meanStep->startTimeStep);
	}
}
...
void WhiteNoiseLinear::RecalculateSigmaScalingCache(StepStruct *sigmaStep) {
	PopInt totalPopulations {neurons->GetTotalPopulations()};
	for (PopInt neuronPop : std::ranges::views::iota(0,totalPopulations)){
		sigmaScalingCache.at(neuronPop) = (sigmaStep->parameterValues.at(neuronPop+totalPopulations)-sigmaStep->parameterValues.at(neuronPop))/(sigmaStep->endTimeStep-sigmaStep->startTimeStep);
	}
}
