...
UncorrelatedPoissonLikeStimulus::UncorrelatedPoissonLikeStimulus(std::shared_ptr<NeuronPopSample> neurons, std::vector<FileEntry> &stimulusParameters,
...
void UncorrelatedPoissonLikeStimulus::SaveParameters(std::ofstream &wParameterStream) const {
  ...
  wParameterStream
      << "#\t\t" << IDstringUncorrelatedStimulus
      << ": noExternalNeurons external neurons with (poisson) firing rate defined in stimulus_step are connected with population X with strength J_X.\n";
}
