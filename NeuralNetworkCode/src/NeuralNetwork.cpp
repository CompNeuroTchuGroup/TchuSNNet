...
void NeuralNetwork::SaveParameters(){
	...
	wParameterStream <<  "globalSeed                  " << std::to_string(infoGlobal.globalSeed)  << " \t\t\t\t#overrides all other seeds if unequal -1\n";

	wParameterStream << "#*****************************************************************\n";
	wParameterStream << "#****************        Spatial parameters        ***************\n";
	wParameterStream << "#*****************************************************************\n";
	wParameterStream << "density                     " << std::to_string(infoGlobal.density) << " \t\t\t\t#total number of neurons/mm^2 or /mm depending on Dimensions \n";
    ...
    wParameterStream <<  "#*****************************************************************\n";
    wParameterStream <<  "scalingSynapticStrength     " << std::to_string(infoGlobal.networkScaling_synStrength)  << "\t\t#Scaling exponent. Set = 0 if no scaling needed, otherwise typical exponent is -0.5.\n";
    wParameterStream <<  "scaling_C_N                 " << std::to_string(infoGlobal.networkScaling_mode)  << "\t\t\t\t# Set = 0 to scale with number of presynaptic neurons C. Set = 1 to scale with total number of neurons N. (details below) \n";
    wParameterStream <<  "#\t\tscaling_C_N=0    scales internal synaptic strengths and UncorrelatedStimulus with C^s    and    WhiteNoiseStimulus and SpatialGaussianStimulus with 1 \n";
    wParameterStream <<  "#\t\tscaling_C_N=1    scales internal synaptic strengths and UncorrelatedStimulus with N^s    and    WhiteNoiseRescaled and SpatialGaussianStimulus with N^(-s) \n";
...
void NeuralNetwork::SaveParameterOptions(){// This function should have stuff moved to the heap (and deleted at the end of the function)
	...
	mockInfo_ptr->globalSeed = 1;//because many of the classes do not allow a negative seed, the ParameterOptions file is generated with an adapted globalInfo
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop;placeholderString += "5.0 ";}
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop;placeholderString += "1.0 "; }
	stimulusString.push_back(SplitStringToEntry(placeholderString + std::to_string(infoGlobal.simulationTime)));
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop; placeholderString += std::to_string((static_cast<double>(neurons->GetTotalPopulations())+1)/10)+ " "; }
    ...
    std::unique_ptr<UncorrelatedPoissonLikeStimulus> uncorrelatedPoissonStimulus = std::make_unique<UncorrelatedPoissonLikeStimulus>(neurons,stimulusString,mockInfo_ptr);
    uncorrelatedPoissonStimulus->SaveParameters(streamPOptions);


	std::vector<FileEntry> whiteNoiseLinearStrings;
	placeholderString = "stimulus_meanCurrent ";
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop; placeholderString += "0.0 "; }
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop; placeholderString += "5.0 "; }
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) { (void)neuronPop;placeholderString += "0.01 "; }
	spatialGaussianStimulusStrings.push_back(SplitStringToEntry(placeholderString + std::to_string(infoGlobal.simulationTime)));
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) { (void)neuronPop;placeholderString += "1.00 "; }
	spatialGaussianStimulusStrings.push_back(SplitStringToEntry(placeholderString + std::to_string(infoGlobal.simulationTime)));
	std::unique_ptr<SpatialGaussianStimulus> SpatialGaussianStim= std::make_unique<SpatialGaussianStimulus>(neurons, spatialGaussianStimulusStrings, mockInfo_ptr);
	...
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) {(void)neuronPop; placeholderString += "0.25 "; }
	stimulusString.push_back(SplitStringToEntry(placeholderString));
	placeholderString = "ExternalConnectivity_PeakProbability ";
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) { (void)neuronPop;placeholderString += "0.8 "; }
	stimulusString.push_back(SplitStringToEntry(placeholderString));
	placeholderString = "stimulus_J_X ";
	for (PopInt neuronPop : std::ranges::views::iota (0, neurons->GetTotalPopulations())) { placeholderString = placeholderString + std::to_string((static_cast<double>(neuronPop) + 1) / 10) + " "; }
	stimulusString.push_back(SplitStringToEntry(placeholderString));
    ...
    std::unique_ptr<RandomConnectivity>  geometryRandomConnectivity = std::make_unique<RandomConnectivity>(static_cast<Synapse*>(synapse_current.get()), mockInfo_ptr);
    geometryRandomConnectivity->SaveParameters(streamPOptions, synapseString);

    streamPOptions << "#************************************************\n";
    std::unique_ptr<AdjacencyMatrixConnectivity> geometryAdjacencyMatrix = std::make_unique<AdjacencyMatrixConnectivity>(static_cast<Synapse*>(synapse_current.get()), mockInfo_ptr);
    geometryAdjacencyMatrix->SaveParameters(streamPOptions, synapseString);

    streamPOptions <<  "#************************************************\n";
    std::unique_ptr<PoissonConnectivity> geometryPoissonConnectivity = std::make_unique<PoissonConnectivity>(static_cast<Synapse*>(synapse_current.get()),mockInfo_ptr);
    geometryPoissonConnectivity->SaveParameters(streamPOptions,synapseString);
	streamPOptions << "#************************************************\n";
	std::unique_ptr<DistanceConnectivity> geometryDistanceConnectivity = std::make_unique<DistanceConnectivity>(static_cast<Synapse*>(synapse_current.get()), mockInfo_ptr);
    ...
    streamPOptions <<  "#************************************************\n";
    std::unique_ptr<MongilloSynapseContinuous> mongilloSynapseContinuous = std::make_unique<MongilloSynapseContinuous>(neuronLIF,neuronLIF,mockInfo_ptr);
    ...
    streamPOptions <<  "#************************************************\n";
    std::unique_ptr<ExponentialCurrentSynapse> exponentialCurrentSynapse = std::make_unique<ExponentialCurrentSynapse>(neuronLIF,neuronLIF,mockInfo_ptr);
    ...
    std::unique_ptr<TraceRBranchedHSTDP> brhSTDP = std::make_unique<TraceRBranchedHSTDP>(mockInfo_ptr);
...
void NeuralNetwork::LoadParameters(std::string baseDirectory,std::vector<FileEntry>& parameterEntries){
    ...
    for (FileEntry& parameterEntry : parameterEntries) {
        ...
        } else if((parameterEntry.parameterName.find("dt_timestep") != std::string::npos || parameterEntry.parameterName.find("dt") != std::string::npos)&& !timestepBool){
    ...
    this->recorder  = std::make_shared<Recorder>(neurons,synapses,stimulus,baseDirectory,recorderParameters,simulationTitle,nonIterateTitle,&infoGlobal);
}
...
//The 5th of November
    while(infoGlobal.timeStep <= simSteps) {
        ...
        /*        double accumulator{};
        for (std::vector<double>& popSynaptic_dV : synaptic_dV) {
            accumulator = std::accumulate(popSynaptic_dV.begin(), popSynaptic_dV.end(), accumulator, [](double accumulator, double value) {return accumulator + value; });
        }
        ...
        if(infoGlobal.timeStep%(static_cast<long>(simSteps*0.001)) == 1){
            ...
            std::cout <<std::fixed<<std::setprecision(1)<< timestepProgressRatio*100 << "%  -- Comp. time: " << computationTime.count() << "/" << static_cast<int>(computationTime.count()/timestepProgressRatio) << " sec. -- " << std::endl;
        }
