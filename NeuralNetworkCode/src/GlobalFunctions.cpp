...
std::vector<FileEntry> FilterStringEntries(const std::vector<FileEntry> &fullEntryVector, std::string filter) {
  ...
  std::copy_if(fullEntryVector.begin(), fullEntryVector.end(), std::back_inserter(filteredEntries),
               [filter](const FileEntry &entry) { return entry.parameterNameContains(filter); });
