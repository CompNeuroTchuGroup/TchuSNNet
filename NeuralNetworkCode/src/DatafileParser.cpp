...
DatafileParser::DatafileParser(Recorder& recorder){ //revised
    ...
    if(recorder.recordRasterPlot){
        for (PopInt neuronPop : neuronPopRasterIds){
            rasterPlotMetadata.emplace_back(recNeuronPopData(recorder.noRasterPlotNeurons.at(neuronPop),recorder.infoGlobal->dtTimestep,static_cast<int>(recorder.infoGlobal->timeStep), neuronPop, recorder.infoGlobal->simulationTime));
        }
...
{
    //population level
    for (PopInt recordedNeuronPop : std::ranges::views::iota(0, static_cast<PopInt>(neuronPopRasterIds.size()))){ //To do proper indexing later, I will need a index-tracing function to get the index in the neuronPopRasterIds vector to index the larger vector properly when reading entries
    //neuron level
        ...
        for (NeuronInt neuron : std::ranges::views::iota(0,rasterPlotMetadata.at(recordedNeuronPop).noNeurons)){
            ...
            tempVector.push_back(std::pair<std::vector<double>, std::pair<PopInt, NeuronInt>>({}, std::pair<PopInt, NeuronInt>(neuronPopRasterIds.at(recordedNeuronPop), neuron)));
        }
        spikeTimesVector.push_back(tempVector);
    }
}

std::vector<std::vector<std::pair<std::vector<double>, std::pair<PopInt, NeuronInt>>>> DatafileParser::parseSpikesToSpikeTimes(std::ifstream &fileStream){ // changing
    //Wrapper 1, assume the file does exist
    std::vector<std::vector<std::pair<std::vector<double>, std::pair<PopInt, NeuronInt>>>> spikeTimesVector; //vector of vectors of size equal to the number of neurons
    ...
    for (FileEntry& parsedEntry : parsedEntries){
        int neuronPopIndex = static_cast<PopInt>(std::distance(neuronPopRasterIds.begin(), std::find(neuronPopRasterIds.begin(),neuronPopRasterIds.end(), std::stoi(parsedEntry.parameterValues.at(1))))); //How do we do exception management here?
    ...
    //Here we can use the FileEntry struct functions to parse a line, then iterate over parameterValues(two different loops). Name will be the timestep, luckily enough. 
...
void DatafileParser::writeSpikeTimesFile(std::vector<std::vector<std::pair<std::vector<double>, std::pair<PopInt, NeuronInt>>>> parsedData, std::string wfilePath, std::vector<recNeuronPopData> metadataVec){//later {
    //Wrapper 2
    std::ofstream stream(wfilePath);
    //Metadata loop
    for (recNeuronPopData singleMetadata : metadataVec){
        stream<<"M_"<<std::to_string(singleMetadata.neuronPopId)<<'='<<std::to_string(singleMetadata.noNeurons)<<','<<std::to_string(singleMetadata.dtTimestep)<<','<<std::to_string(singleMetadata.totalTimesteps)<<','<<std::to_string(singleMetadata.neuronPopId)<<','<<std::to_string(singleMetadata.simTime)<<'\n';
    }
    //Population loop
...
recNeuronPopData::recNeuronPopData(NeuronInt noNeurons, double dtTimestep, int totalTimesteps,PopInt  neuronPopId, double simTime): noNeurons{noNeurons}, dtTimestep{dtTimestep}, simTime{simTime}, totalTimesteps{totalTimesteps}, neuronPopId{neuronPopId}
{
}
