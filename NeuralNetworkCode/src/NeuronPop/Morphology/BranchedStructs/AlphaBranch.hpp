...
struct AlphaBranch : public Branch {
  ...
  // std::vector<int> potentiationCountSTDP{};//Size of these has to be the amount of synapses in the branch (equal to triggerCount, length divided by
  // gaps) //REMOVED due to redundancy Misc
  //  int plasticityBranchEventsThisTimestep{};//Has to be set to zero
  //  std::deque<int> plasticityEventsPerTimestepWindow{};//Every timestep I push_front the events in the timestep and delete the last element with
  //  neuronPop or erase. Then sum and check if over the threshold.
  ...
  AlphaBranch(std::vector<int> anteriorBranches, double gap, double branchLength, int branchId, double preSynTraceDecay, double coopTraceDecay,
              double alphaExpDecay, double alphaStimBump, double alphaBasal, double betaResourcePool, double omegaOffset);
  AlphaBranch(double gap, double branchLength, int branchId, double preSynTraceDecay, double coopTraceDecay, double alphaExpDecay,
