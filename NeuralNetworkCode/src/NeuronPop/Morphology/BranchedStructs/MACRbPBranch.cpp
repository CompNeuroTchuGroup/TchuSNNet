...
MACRbPBranch::MACRbPBranch(std::vector<int> anteriorBranches, double gap, double branchLength, int branchId, MACRbPSynapseSpine spine):
    Branch(anteriorBranches, gap, branchLength, branchId) {
...
MACRbPBranch::MACRbPBranch(double gap, double branchLength, int branchId, MACRbPSynapseSpine spine):
    Branch(gap, branchLength, branchId), MACRbPspines(branchLength / gap, spine) {
  // MACRbPspines.resize(branchLength / gap, spine);
...
void MACRbPBranch::PostConnectSetUp(std::vector<BranchedSpinePtr> spineData) {
  // If it is ever necessary to release the weight of unconnected synapses as free resources, uncomment the next lines
  // of code. PROBLEM WITH THIS: Spines will not be in steady state anymore, they will start over-depressed double
  // extraResources{}; for (const MACRbPSynapseSpine &spine : MACRbPspines) {
  //   if (!spine.connected) {
  ...
  //                [extra_alloc_resources](MACRbPSynapseSpine &spine) { spine.resourcesAvailable +=
  //                extra_alloc_resources; });
}

void MACRbPBranch::Advect(const RuntimeConstants &constants) {
  // A possible ugly optimization is to do the first two iteration containers (if connected), the diffusion of the first
  // container, then iterate from index 2 onwards and do the diffusion of container index-1. When the end is reached,
  // you do the diffusion of last container.
  const RuntimeConstants ctt { constants };  //
  ...
  std::for_each(PAR_UNSEQ, MACRbPspines.begin(), MACRbPspines.end(), [ctt](MACRbPSynapseSpine &spine) {
    ...
    if (spine.enabled) {
      ...
      // ngDot = ctt.reaction1Ctt * (ctt.neurograninTotal - spine.calmodulinNeurogranin) *
      // (inactiveCalmodulin)-ctt.reaction2Ctt *
      ...
      // camDot = ctt.reaction3Ctt * std::pow(spine.calciumFree, 2) * inactiveCalmodulin - ctt.reaction4Ctt *
      // spine.calmodulinActive; spine.calmodulinActive += camDot; spine.calciumFree -= 2 * camDot;
      // // To here is the equilibrium
      //  ORDERING
      //   4th calcineurin binding
      double kDot {}, nDot {}, kPDot {}, wDot {};
      nDot = ctt.reaction5Ctt * (ctt.calcineurinTotal - spine.calcineurinActive) * spine.calmodulinActive -
             ctt.reaction6Ctt * spine.calcineurinActive;
      ...
      // spine.calmodulinActive -= nDot + kDot;//We are not doing this because this could mean negative active
      // calmodulins 8th Change in synapse spine size/weight
      ...
      // / ctt.resourceConversionFct; // This should be the case for converting the dmV/spike to concentration of
      // resources For the next timestep
    }
    spine.PreDiffusion();
#ifndef NDEBUG
  ...
  MACRbPspines.at(0).resourcesAvailable +=
    ctt.resourceDiffusionFct * (-MACRbPspines.at(0).resourcesOldStep + MACRbPspines.at(1).resourcesOldStep);
...
void MACRbPBranch::AdvectUnrolled(const RuntimeConstants &constants) {
  const RuntimeConstants ctt { constants };
  ...
  if (MACRbPspines.at(0).enabled) {
    ...
    MACRbPspines.at(0).preTransient += ctt.preCalciumFluxFactor * MACRbPspines.at(0).preTransientIncrease -
                                       MACRbPspines.at(0).preTransient * ctt.calciumExtrusionCtt;  // This is A in appendix
...
#endif
  size_t lastIndex { MACRbPspines.size() - 1 };
  auto   range = std::ranges::common_view(std::views::iota(1ull) | std::views::take(lastIndex - 1));  // It takes up to that element
  std::for_each(PAR, range.begin(), range.end(), [this, ctt](size_t spineIndex) {
    ...
    if (MACRbPspines.at(spineIndex).enabled) {
      ...
      // Here we add the traces plus the basal flux to the calcium
      MACRbPspines.at(spineIndex).calciumFree +=
        MACRbPspines.at(spineIndex).preTransientIncrease + MACRbPspines.at(spineIndex).postTransientIncrease;
      // Until here, now first reactions
      ...
      double kDot {}, nDot {}, kPDot {}, wDot {};
      nDot = ctt.reaction5Ctt * (ctt.calcineurinTotal - MACRbPspines.at(spineIndex).calcineurinActive) *
  ...
  if (MACRbPspines.at(lastIndex).enabled) {
    ...
    MACRbPspines.at(lastIndex).preTransient += ctt.preCalciumFluxFactor * MACRbPspines.at(lastIndex).preTransientIncrease -
                                               MACRbPspines.at(lastIndex).preTransient * ctt.calciumExtrusionCtt;  // This is A in appendix
    ...
    // Here we add the traces plus the basal flux to the calcium
    MACRbPspines.at(lastIndex).calciumFree +=
      MACRbPspines.at(lastIndex).preTransientIncrease + MACRbPspines.at(lastIndex).postTransientIncrease;
    // Until here, now first reactions
    ...
    double kDot {}, nDot {}, kPDot {}, wDot {};
    nDot = ctt.reaction5Ctt * (ctt.calcineurinTotal - MACRbPspines.at(lastIndex).calcineurinActive) *
...
void MACRbPBranch::PostSpikeCalciumFlux(double nonlinearFactor) {
  for (MACRbPSynapseSpine &spine : MACRbPspines) {
