...
struct MACRbPBranch : public Branch {
    // std::vector<std::vector<double>> waitingMatrix;//First axis is the position and time 0. Second axis is the time
    // 1, 2, 3. Receives calcium of both prespikes and postspikes We swap vector positions (0,1;1,2;2,3), then set to
    // false last vector. std::iter_swap or std::swap. Test speed in this. Synapse access
    ...
    // Input methods
    //  void PreSpikeCalciumInflux(TStepInt&& timestep);//We do swaps of the matrix and fill with zeroes. Matrix
    //  contains the calcium concentration already and we just add. void PreSpikeCalciumInflux();//We do swaps of the
