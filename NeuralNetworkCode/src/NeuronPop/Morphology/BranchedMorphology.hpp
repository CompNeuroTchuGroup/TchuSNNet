...
class BranchedMorphology : public Morphology {
  protected:
    ...
    std::vector<BranchPtr>
      branches {};  // unique_ptr's constructor is explicit, so you either need to use emplace_back or
                    // stuff.push_back(std::unique_ptr<int>(new static_cast<int>(i)));. Between the two, emplace_back is much cleaner.
    std::vector<BranchedSpinePtr>
      branchedSpineData;  // They are just pointers, what is the worst that can happen by having multiple copies?
  ...
  public:
    ...
    // setUp SYnapse slots is called for every branch in a loop and depending on the bool (universal for all branches for now) it calls
    // random or ordered. The overriding function calls functions of BMorpho.
    void SetUpBranchingTree(int &remainingBranches, int remainingBranchings,
                            std::vector<int> anteriorBranches);  // Here we set up the vector with the branches
    ...
    void SetUpSynapseSlots(BranchPtr branch);  // This function will set up the open synapse slots of a branch object with its id.This one I
                                               // have to define in the parallel synaptic connectivity masks or the derived classes
    ...
    int AllocateBranch(
      const BranchTargeting synapse);  // The selected branch allocation is simple. This function is called in AllocateNewSynapse
    ...
    void OrderedSynapseAllocation(
      BranchPtr branch);  // These two are coming from the SetUpSynapseSlots already, called depending on a bool.
    ...
    // int GetMaxGapDelay(int delayPerMicroMeter) override;
};
