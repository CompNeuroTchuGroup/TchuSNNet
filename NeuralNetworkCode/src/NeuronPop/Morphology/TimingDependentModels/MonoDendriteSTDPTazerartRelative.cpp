...
MonoDendriteSTDPTazerartRelative::MonoDendriteSTDPTazerartRelative(GlobalSimInfo *infoGlobal, const std::vector<FileEntry> &morphologyParameters)
...
void MonoDendriteSTDPTazerartRelative::UpdateLTP(signed long spineID) {
  ...
  this->spineDataCoop.at(spineID)->weight += (this->spineDataCoop.at(spineID)->GetWeightUncoupled() * this->preFactorLTP *
                                              this->aLTP(spine->GetTheta()) * this->gLTP(this->lastPostSpikeTime - spine->GetLastSpike()));
...
void MonoDendriteSTDPTazerartRelative::UpdateLTD(signed long spineID) {
  CoopSynapseSpine *spine = this->spineDataCoop.at(spineID).get();
  //    this->weightsSum -= this->synapseDataCoop.at(synId)->weight;
  this->spineDataCoop.at(spineID)->weight += (-this->spineDataCoop.at(spineID)->GetWeightUncoupled() * this->preFactorLTD *
                                              this->aLTD(spine->GetTheta()) * this->gLTD(spine->GetLastSpike() - this->lastPostSpikeTime));
