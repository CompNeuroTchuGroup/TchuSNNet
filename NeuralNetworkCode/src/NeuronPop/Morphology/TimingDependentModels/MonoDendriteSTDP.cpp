...
void MonoDendriteSTDP::Advect() {
  ...
  // Recording plasticity events here makes no sense, because every time-step all thetas change and all weights change, and the framework is not
  // event-based.
  ...
  for (signed long spineID1 : std::ranges::views::iota(0, static_cast<signed long>(spikedSpinesId.size()))) {
    for (signed long spineID2 :
         std::ranges::views::iota(spineID1 + 1, static_cast<signed long>(spikedSpinesId.size()))) { // Not permutation loop, it is triangular.
...
void MonoDendriteSTDP::WeightDecay() {
  if (this->decayWeights) {
    std::for_each(baseSpineData.begin(), baseSpineData.end(),
                  [this](BaseSpinePtr spine) { spine->weight = (spine->GetWeightUncoupled() * weightExpDecay); });
  }
}
void MonoDendriteSTDP::ThetaDecay() {
  std::for_each(spineDataCoop.begin(), spineDataCoop.end(),
                [this](const CoopSpinePtr &spine) { spine->SetTheta(spine->GetTheta() * thetaExpDecay); });
}
...
void MonoDendriteSTDP::RecordExcitatoryPreSpike(BaseSpinePtr spinePtr) {
  ...
  this->spineDataCoop.at(spinePtr->idInMorpho)
      ->SetLastSpike(static_cast<double>(this->infoGlobal->timeStep) * this->infoGlobal->dtTimestep); // only coop
...
void MonoDendriteSTDP::SaveParameters(std::ofstream &wParameterFile, std::string neuronIdentificator) const {
  ...
  wParameterFile << "\t"
                 << "#Length of the dendritic arm (μm).\n";

  wParameterFile << neuronIdentificator << "synapticGap\t\t\t" << std::to_string(this->synapticGap);
  wParameterFile << "\t"
                 << "#Forced distance between synapses (μm).\n";

  wParameterFile << neuronIdentificator << "heterosynapticThetaDecay\t\t" << std::to_string(this->tauTheta);
  wParameterFile << "\t"
                 << "#Or tauTheta, decay constant of heterosynaptic effects in spines.\n";

  wParameterFile << neuronIdentificator << "intersynapseDistanceDecay\t\t" << std::to_string(this->lambdaDist);
  wParameterFile << "\t"
                 << "#Or lambdaDist, spatial decay constant of heterosynaptic boost between synapses.\n";

  wParameterFile << neuronIdentificator << "intersynapseSpikeTimingDecay\t" << std::to_string(this->tauDelay);
  wParameterFile << "\t"
                 << "#Or tauDelay, decay constant of heterosynaptic effects over inter-synapse spike timing difference.\n";

  wParameterFile << neuronIdentificator << "preFactorLtp\t\t\t" << std::to_string(this->preFactorLTP);
  wParameterFile << "\t"
                 << "#Base factor that is multiplied by the spatio-temporal effects in LTP. If set to zero, LTP will be zero. \"A\" equivalent\n";

  wParameterFile << neuronIdentificator << "preFactorLtd\t\t\t" << std::to_string(this->preFactorLTD);
  wParameterFile << "\t"
                 << "#Base factor that is multiplied by the spatio-temporal effects in LTD. If set to zero, LTD will be zero. \"A\" equivalent\n";

  wParameterFile << neuronIdentificator << "incrLtp\t\t\t\t" << std::to_string(this->incrementLTP);
  wParameterFile << "\t"
                 << "#Max possible increase in LTP due to cooperativity . \"I\" equivalent\n";

  wParameterFile << neuronIdentificator << "decrLtd\t\t\t\t" << std::to_string(this->decrementLTD);
  wParameterFile << "\t"
                 << "#Max possible LTD due to cooperativity.  \"D\" equivalent\n";

  wParameterFile << neuronIdentificator << "baseLtp\t\t\t\t" << std::to_string(this->baseLTP);
  wParameterFile << "\t"
                 << "#Default increase in weight per LTP check.  \"B\" equivalent\n";

  wParameterFile << neuronIdentificator << "baseLtd\t\t\t\t" << std::to_string(this->baseLTD);
  wParameterFile << "\t"
                 << "#Default decrease of weight per LTD check. \"B\" equivalent\n";

  wParameterFile << neuronIdentificator << "weightDecay\t\t\t" << std::boolalpha << this->decayWeights << std::noboolalpha << "\t"
                 << std::setprecision(3) << std::to_string(this->WeightDecayConstant) << std::setprecision(6);
  wParameterFile
      << "\t"
      << "#The first bool activates the weight decay per timestep. The second number is the time constant on an exponential in seconds [exp(-dt/ctt)].\n";

  wParameterFile << neuronIdentificator << "minmaxWeights\t\t\t" << std::setprecision(3) << std::to_string(this->minWeight) << " "
                 << std::to_string(this->maxWeight) << std::setprecision(6);
  wParameterFile
      << "\t"
      << "#Only relevant for HardNormalization and distributeWeights, the first number is the minimum weight in normalization, the second one the hard cap for weight.\n";
  wParameterFile << neuronIdentificator << "alphaLtp\t\t\t\t" << std::to_string(this->alpha);
  wParameterFile << "\t"
                 << "#Cooperativity decay for LTP.\n";
  wParameterFile << neuronIdentificator << "betaLtd\t\t\t\t" << std::to_string(this->beta);
  wParameterFile << "\t"
                 << "#Cooperativity decay for LTD.\n";
}
...
void MonoDendriteSTDP::LoadParameters(const std::vector<FileEntry> &morphologyParameters) {
  ...
  bool branchLengthInitiallized{false}, synapticGapInitialized{false}, tauThetaInitialized{false}, lambdaDistInitialized{false},
      tauDelayInitialized{false}, baseLtpInitialized{false}, incrLtpInitialized{false}, baseLtdInitialized{false}, decrLtdInitialized{false},
      alphaInitialized{false}, betaInitialized{false}, normalizationFound{false};
...
void MonoDendriteSTDP::SoftMaxNormalize() {
  ...
  double weightSum = std::accumulate(this->baseSpineData.begin(), this->baseSpineData.end(), 0.0,
                                     [](double weightSum, BaseSpinePtr synapse) { return weightSum += std::exp(synapse->GetWeightUncoupled()); });
  // for (auto& syn : this->baseSpineData) {
  //     sumWeights += std::exp(syn->GetWeight() - maxWeights);
  // }

  // It is not clear if the following lines are correct in Softmax normalization. There was no reference previously, so Toni assumed the normalization
  // is the one done in NNs. As for the multiplication by two, this is because the weights in Saif models are normally distributed between 0 and 2.
  // This can be changed in a model with an extra entry in LP
...
BaseSpinePtr MonoDendriteSTDP::AllocateNewSynapse(BranchTargeting &branchTargeting) {
  ...
  if (this->nextPos <= this->dendriticLength) { // This is very badly thought. The synapse allocation should throw, and not allow the programme to
                                                // keep going with an unresolved issue
...
void MonoDendriteSTDP::UpdateCooperativity(signed long spikerId, signed long neighborId) {
  ...
  double hEffects =
      getDistanceEffects(spiker, neighbor) * getTimingEffects(spiker, neighbor) * spiker->GetWeightUncoupled() * neighbor->GetWeightUncoupled();
  spiker->AddToTheta(hEffects);
  neighbor->AddToTheta(hEffects);

  //    if (hEffects != 0.0) {//OPTIMIZATION. Problems with heap allocation of the data (causes an OOM error). Needs to be moved to a file if
  //    necessary
