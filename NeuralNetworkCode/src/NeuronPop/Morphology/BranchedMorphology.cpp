...
void BranchedMorphology::SaveParameters(std::ofstream &wParameterFile, std::string neuronIdentificator) const {
    ...
    wParameterFile << neuronIdentificator << "distributeWeights\t\t" << std::boolalpha << this->distributeWeights
                   << std::noboolalpha << " #μm";
    wParameterFile << "\t"
                   << "#Distance between synapse spines.\n";
    ...
    wParameterFile << neuronIdentificator << "tree\t\t" << std::boolalpha << this->branchingTreePattern
                   << std::noboolalpha
...
int BranchedMorphology::AllocateBranch(const BranchTargeting branchTarget) {
    if (branchTarget.setTargetBranch &&
        !(branchTarget.targetBranch >=
          static_cast<int>(this->branches.size()))) { // SHould this condition be equal or less than?
...
void BranchedMorphology::RandomSynapseAllocation(BranchPtr branch) {
    ...
    std::sample(possibleSlots.begin(), possibleSlots.end(), std::back_inserter(branch->openSpineSlots),
                branch->branchSlots, generator);
...
void BranchedMorphology::SetUpBranchingTree(int &remainingBranches, int remainingBranchings,
...
void BranchedMorphology::SetUpBranchingBrush(int &remainingBranches, int &remainingBranchings,
                                             std::vector<int> anteriorBranches) {
    // This is a recursive function that sets up the branched dendritic tree and is generalized for 0 branchings (1
    // branch). This function has been unit tested by Antoni.
    // If the variable remainingBranchings is changed to a reference, the tree will be a brush.
...
void BranchedMorphology::PostConnectSetUp() {
    ...
    std::for_each(branches.begin(), branches.end(),
                  [this](BranchPtr branch) { branch->PostConnectSetUp(branchedSpineData); });
}
