...
void AlphaResourceHSTDP::Advect() {
  ...
  if (this->postSpiked) {
    ...
    std::for_each(alphaBranches.begin(), alphaBranches.end(),
                  [this](AlphaBranch &branch) { branch.ApplyTracesOnSpinesLTD(this->postSynapticTrace, this->biasLTD); });
  }
  // ApplyEffects();
  Reset();
}
...
void AlphaResourceHSTDP::ApplyCoopTraceSpatialProfile(int branchSpineID, AlphaBranch &currentBranch) {
...
void AlphaResourceHSTDP::RecordExcitatoryPreSpike(BaseSpinePtr spinePtr) {
  ...
  alphaBranches.at(static_cast<AlphaSpinePtr>(spinePtr)->branchId)
      .spikedSpinesInTheBranch.push_back(static_cast<AlphaSpinePtr>(spinePtr)->branchPositionId);
  // branch.preSynapticTraces.at(branchSpinePosition) += 1;
  // ApplyCoopTraceSpatialProfile(branchSpinePosition, branch);
