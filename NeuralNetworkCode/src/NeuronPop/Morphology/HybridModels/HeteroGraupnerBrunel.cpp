...
void HeteroGraupnerBrunel::SaveParameters(std::ofstream &wParameterFile, std::string neuronIdentificator) const {
  ...
  wParameterFile << neuronIdentificator << "betaResourcePool\t\t" << std::to_string(this->betaResourcePool * noBranches);
  wParameterFile << "\t"
...
void HeteroGraupnerBrunel::SetUpHashTable() {
  ...
  //      throw "The timeKernel lenght is assumed to be longer than the spatialKernel length."; //The timeKernel
  //      lenght is always thought to be longer than the spatial one. 1 ms timing for 10 um is very one-sided, and
  //      that is equilibrium
  //  }
...
void HeteroGraupnerBrunel::Advect() {
  ...
  if (this->postSpiked) {
    ...
    std::for_each(alphaBranches.begin(), alphaBranches.end(), [this](AlphaBranch &branch) { branch.ApplyTracesOnSpinesLTP(); });
    ...
    std::for_each(alphaBranches.begin(), alphaBranches.end(),
                  [this](AlphaBranch &branch) { branch.ApplyTracesOnSpinesLTD(this->GetPostSynapticTrace(), GetLTDBias()); });
  }
  // ApplyEffects();
  Reset();
}

// void AlphaResourceHSTDP::ApplyEffects() { //Called after pairings
//     //If first precount is bigger than twice the STDP depression count, depression. Otherwise potentiation.

// }

// void AlphaResourceHSTDP::UpdateCoopTrace(const ResourceTraceBranch* const branch) {//These are the spiked neurons on
// synapseDataIndexes
//     //Here we call SetTempEffects if __it__ happens
//     std::for_each(branch->spikedSpinesInTheBranch.begin(), branch->spikedSpinesInTheBranch.end(), [branch, this](int
//     spineIDinBranch){
...
// {
//     //This function raises an overflow flag in compilation due to operations before casting to ptrdiff_t (long long).
//     There should never be overflow from a design standpoint return
//     std::count_if(std::max(branch->triggerCount.begin(),
//     std::next(branch->triggerCount.begin(),synapseIDinBranch-kernelGapNumber)),std::min(branch->triggerCount.end(),
//     std::next(branch->triggerCount.begin(),synapseIDinBranch+kernelGapNumber+1)), [this](int pairingCounter){return
//     pairingCounter<this->timeKernelLength;})>2;
// }

void HeteroGraupnerBrunel::ApplyCoopTraceSpatialProfile(int branchSpineID, int branchID) {
  ...
  std::vector<double>::iterator forwardBegin = alphaBranches.at(branchID).cooperativityTraces.begin() + (branchSpineID + 1);
  std::vector<double>::iterator forwardEnd   = alphaBranches.at(branchID).cooperativityTraces.end();
  ...
  //  to the end of the branch, as represented by branchID-branchID+branchSlots-1=branchSlots-1, last index
  ...
  //      // } else {
  //      //     ResourceSpinePtr neighbourSynapseSpine =
  //      currentBranch->rBranchSpineData.at(spinePositionBranchIndex);
  ...
  //      //     // if (timeStepDifference<=STDPDepressionCount){//Indicates depression region with no conflict. If
  //      the spike is far away, but too far for depression, nothing will happen. If a postspike happens, these flags
  //      are ignored.
  //      //     //     //The boolean is <= to classify the pairing where the postspike and first prespike are on the
  //      same timestep as depression.
  //      //     //     // centralSynapseSpine->SetDepressionFlag(true);
  //      //     //     // neighbourSynapseSpine->SetDepressionFlag(true);
  //      //     //     //alphaStimulusEffect *= -DecayHashTableSTDP.at(STDPDepressionCount-timeStepDifference);//This
  //      is to apply a decay equivalent to the STDP kernel to the first pre-spike with depression (*-1) (calcium
  //      accumulation oriented)
  //      //     // } else { //Knowing depression region, indicates conflict that is skewed towards potentiation
  //      according to STDP kernel
  //      //     //     if (timeStepDifference<=2*STDPDepressionCount){//We set the boolean to equal too to solve the
  //      conflict in favor of potentiation, as depression has the other fringe case
  //      //     //         centralSynapseSpine->SetPotentiationFlag(true);
  //      //     //         neighbourSynapseSpine->SetPotentiationFlag(true);
  //      //     //         alphaStimulusEffect *=
  //      DecayHashTableSTDP.at(timeStepDifference-STDPDepressionCount);//This is to apply a decay equivalent to the
  //      STDP kernel to the first pre-spike  (calcium accumulation oriented)
  //      //     //     } else {
  //      //     //         centralSynapseSpine->SetDepressionFlag(true);
  //      //     //         neighbourSynapseSpine->SetDepressionFlag(true);
  ...
  //      //     // }
  //      //     if(timeStepDifference+timeStepsPerSynGap*absDistance<=kernelRadiusArea){ //&&
  //      (kernelizedSynapses.find(synapsePositionIndexInBranch) == kernelizedSynapses.end())){
  //      //triggercount+distance gap*spaceTimeStepRelation<maxCountTime + 1*spaceTimeStepRelation (what should
  //      constrain the triangular matrix)
  //      //         //The postspiked condition is because the STDP kernel is discretized, in the case the postspike
  //      happens at the same time as the pairing of a synapse, the pairing does not happen (as the potentiation and
  //      depression would counteract themselves)
  //      //         //Avoiding the check of whether the neighbour was already updated assumes that no two spines will
  //      fire the same dt.
  ...
  //      //         spikedSpine->AddTempResourcesToSpine(alphaStimulusEffect); //Removing this line (plus updated
  //      alpha effects) will change the model from pairing to cooperativity only.
  ...
  std::vector<double>::reverse_iterator reverseBegin =
      alphaBranches.at(branchID).cooperativityTraces.rbegin() + (alphaBranches.at(branchID).cooperativityTraces.size() - branchSpineID);
  std::vector<double>::reverse_iterator reverseEnd = alphaBranches.at(branchID).cooperativityTraces.rend();
...
// }
// double AlphaResourceHSTDP::CallKernelHashTable(int distanceToCenterInGaps, int timeDifference) {
//     //Matrix is symmetric!! Only have to do half!
//     //Here we tabulate for every distance from center a equivalency between counts and effects (reverse as counts go
//     up to maxCount)
...
// void AlphaResourceHSTDP::ClearSynapseSets() {
...
void HeteroGraupnerBrunel::PostConnectSetUp() {
  ...
  if (distributeWeights) {
    ...
    for (AlphaSpinePtr spine : resourceSpineData) {
      spine->alphaResources = (weightDistribution(generator)) + alphaBasal;
    }
  }
}
...
BaseSpinePtr HeteroGraupnerBrunel::AllocateNewSynapse(BranchTargeting &branchTarget) {
  ...
  newSpine->idInMorpho = (this->baseSpineData.size());
  // Branch
...
std::vector<double> HeteroGraupnerBrunel::GetOverallSynapticProfile() const {
  ...
  dataArray.at(0) = std::accumulate(this->baseSpineData.begin(), this->baseSpineData.end(), 0.0,
                                    [](double accumulator, const BaseSpinePtr syn) { return accumulator + syn->GetWeightUncoupled(); }) /
                    noSpines;
