...
MACRbPModel::MACRbPModel(GlobalSimInfo *infoGlobal, const std::vector<FileEntry> &morphologyParameters,
...
void MACRbPModel::LoadParameters(const std::vector<FileEntry> &morphologyParameters) {
  ...
  for (auto &[parameterName, parameterValues] : morphologyParameters) {
    if (parameterName.find("kinasesTotal") != std::string::npos) {
      ...
      this->constants.resourceDiffusionFct =
...
void MACRbPModel::PreCalculateParameters() {
  this->constants.reaction1234Ctt = this->constants.reaction1Ctt * this->constants.reaction4Ctt /
                                    (this->constants.reaction2Ctt * this->constants.reaction3Ctt);
  this->constants.calciumInfluxBasal = calciumBasal * constants.calciumExtrusionCtt;
  this->constants.initialResources =
    this->constants.initialWeight / constants.resourceConversionFct / availResourcesRatio;  // REVIEW
  // this->constants.reaction11Ctt *= constants.resourceConversionFct; // REVIEW this->constants.preCalciumRiseRate   =
  // 1 / preCalciumRiseTau;
  ...
  auto amplitudeNormalization = [](double amplitude, double calcium_tau,
    ...
    return amplitude /
           (calcium_tau * std::exp((-calcium_tau * std::log(calcium_tau / decay_tau)) / (calcium_tau - decay_tau)));
  };  // Checked in python, this is valid
  // calciumExtrusion is a rate, not a tau
  this->constants.preCalciumFluxFactor =
    amplitudeNormalization(prespikeCalcium, 1 / this->constants.calciumExtrusionCtt, preCalciumDecayTau);
  this->constants.postCalciumFluxFactor =
    amplitudeNormalization(postspikeCalcium, 1 / this->constants.calciumExtrusionCtt, postCalciumDecayTau);
}
...
void MACRbPModel::SaveParameters(std::ofstream &wParameterFile, std::string neuronIdentificator) const {
  ...
  wParameterFile << neuronIdentificator << "phosphatasesTotal\t\t" << (this->constants.calcineurinTotal)
                 << " #uM/spine";
  wParameterFile << "\t"
  ...
  wParameterFile << neuronIdentificator << "kOne\t\t" << (this->constants.reaction1Ctt / infoGlobal->dtTimestep)
                 << " #uM^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From CaM and Ng to CaMNg\n";
  wParameterFile << neuronIdentificator << "kTwo\t\t" << (this->constants.reaction2Ctt / infoGlobal->dtTimestep)
                 << " #s^{-1}";
  wParameterFile << "\t"
                 << "#From CaMNg to CaM and Ng\n";
  wParameterFile << neuronIdentificator << "kThree\t\t" << (this->constants.reaction3Ctt / infoGlobal->dtTimestep)
                 << " #uM^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From CaM and Ca to active CaM\n";
  wParameterFile << neuronIdentificator << "kFour\t\t" << (this->constants.reaction4Ctt / infoGlobal->dtTimestep)
                 << " #s^{-1}";
  wParameterFile << "\t"
                 << "#From active CaM to CaM and Ca\n";
  wParameterFile << neuronIdentificator << "kFive\t\t" << (this->constants.reaction5Ctt / infoGlobal->dtTimestep)
                 << " #uM^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From N inactive to N active, using CaM\n";
  wParameterFile << neuronIdentificator << "kSix\t\t" << (this->constants.reaction6Ctt / infoGlobal->dtTimestep)
                 << " #s^{-1}";
  wParameterFile << "\t"
                 << "#From N active to N inactive, using CaM\n";
  wParameterFile << neuronIdentificator << "kSeven\t\t" << (this->constants.reaction7Ctt / infoGlobal->dtTimestep)
                 << " #uM^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From K bound to CaM to phosphorylated K\n";
  wParameterFile << neuronIdentificator << "kEight\t\t" << (this->constants.reaction8Ctt / infoGlobal->dtTimestep)
                 << " #u M^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From phosphorylated K to K bound to CaM\n";
  wParameterFile << neuronIdentificator << "kNine\t\t" << (this->constants.reaction9Ctt / infoGlobal->dtTimestep)
                 << " #u M^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From K inactive to K active, using CaM\n";
  wParameterFile << neuronIdentificator << "kTen\t\t" << (this->constants.reaction10Ctt / infoGlobal->dtTimestep)
                 << " #u M^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From K active to K inactive, using CaM\n";
  wParameterFile << neuronIdentificator << "kEleven\t\t" << ((this->constants.reaction11Ctt) / infoGlobal->dtTimestep)
                 << " #u M^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From resources to weight\n";
  wParameterFile << neuronIdentificator << "kTwelve\t\t" << (this->constants.reaction12Ctt / infoGlobal->dtTimestep)
                 << " #u M^{-1} s^{-1}";
  wParameterFile << "\t"
                 << "#From weight to resources\n";

  wParameterFile << neuronIdentificator << "resourceDiffusion\t"
                 << (this->constants.resourceDiffusionFct * std::pow(synapticGap, 2) / infoGlobal->dtTimestep)
                 << " #um^{2} s^{-1}";
  wParameterFile << "\t"
                 << "#Diffusion constant of resources used to increase synapse size\n";
  wParameterFile << neuronIdentificator << "calciumDiffusion\t"
                 << (this->constants.caDiffusionFct * std::pow(synapticGap, 2) / infoGlobal->dtTimestep)
                 << " #um^{2} s^{-1}";
  wParameterFile << "\t"
  ...
  wParameterFile << neuronIdentificator << "preCalciumDecayTau\t\t"
                 << (static_cast<double>(this->preCalciumDecayTau) * infoGlobal->dtTimestep) << " #secs.";
  wParameterFile << "\t"
                 << "#Delay of the prespike calcium influx\n";
  ...
  wParameterFile << neuronIdentificator << "postCalciumDecayTau\t\t"
                 << (static_cast<double>(this->postCalciumDecayTau) * infoGlobal->dtTimestep) << " #secs.";
  wParameterFile << "\t"
                 << "#Delay of the prespike calcium influx\n";
  wParameterFile << neuronIdentificator << "nonlinearNMDAFactor\t\t"
                 << (static_cast<double>(this->constants.nonlinearFactorNMDA)) << " #-";
  wParameterFile << "\t"
                 << "#Delay of the prespike calcium influx\n";
  ...
  wParameterFile << "\t"
                 << "#Delay of the prespike calcium influx\n";
  wParameterFile << neuronIdentificator << "calciumExtrusion\t\t"
                 << (this->constants.calciumExtrusionCtt / infoGlobal->dtTimestep) << " #s^{-1}";
  wParameterFile << "\t"
                 << "#Extrusion rate for free calcium\n";
  wParameterFile << neuronIdentificator << "resourceConversionFct\t\t" << (constants.resourceConversionFct)
                 << " #mV spike^{-1} uM^{-1}";
  wParameterFile << "\t"
...
MACRbPSynapseSpine MACRbPModel::ComputeSteadyState(MACRbPSynapseSpine &spine) {
  ...
  while (std::abs(spine.resourcesAvailable - spine.resourcesOldStep) > std::numeric_limits<double>::epsilon()) {
    ...
    for (int i = 0; i < constants.newtonIterations; i++) {
      epsilon = (constants.reaction1234Ctt * freeNg * spine.calmodulinActive -
                 spine.calmodulinNeurogranin * std::pow(spine.calciumFree, 2)) /
                (constants.reaction1234Ctt * (freeNg + spine.calmodulinActive) +
    ...
    kDot = constants.reaction9Ctt * (constants.kinasesTotal - spine.kinasesCaM - spine.kinasesPhospho) *
             spine.calmodulinActive -
    ...
    // / constants.resourceConversionFct; // This should be the case for converting the dmV/spike to concentration of
    // resources Store equilibrium in a file? Together with time spent in the calculation? Output only to terminal the
    // time?
  }
...
int MACRbPModel::CreateBranch(std::vector<int> anteriorBranches) {
  ...
  if (!anteriorBranches.empty()) {
    this->MACRbPBranches.push_back(
      MACRbPBranch(anteriorBranches, this->synapticGap, this->branchLength, branchId, constants,
    ...
    int branchId { this->GenerateBranchId() };
    this->MACRbPBranches.push_back(
      MACRbPBranch(this->synapticGap, this->branchLength, branchId, constants,
                   steady_state_spine));  // This vector should be sorted by ID by default (tested).
    this->branches.push_back(static_cast<Branch *>(&this->MACRbPBranches.back()));
  }
  return branchId;
...
BaseSpinePtr MACRbPModel::AllocateNewSynapse(BranchTargeting &branchTarget) {
  ...
  // newSpine->weight     = constants.initialWeight / constants.resourceConversionFct;//Not necessary, weight is set in
  // the SS computation. This would be an active bug Branch
