...
class MACRbPModel : public BranchedMorphology {
  ...
  protected:
    ...
    // TStepInt TStepModded{};           // Here we store the modulus (only calculated once). We will calculate it every
    // timestep just for safety. TStepInt TStepInput{};//Here is where the input happens

  public:
    // main Methods
    MACRbPModel() = default;
    explicit MACRbPModel(GlobalSimInfo *infoGlobal, const std::vector<FileEntry> &morphologyParameters,
    ...
    BaseSpinePtr AllocateNewSynapse(
      BranchTargeting &branchTarget) override;  // Call the Branched one inside before setting all counters
