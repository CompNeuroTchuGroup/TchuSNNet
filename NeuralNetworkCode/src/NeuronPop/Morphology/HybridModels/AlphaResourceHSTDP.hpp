...
class AlphaResourceHSTDP : public BranchedMorphology {
  ...
  protected:
    ...
    std::vector<double> spatialProfile {};  // This is more efficient than the map, but we need a hash function. TEST whether this is faster
                                            // than vector<vector> or map<>
  ...
  public:
    ...
    explicit AlphaResourceHSTDP(GlobalSimInfo *infoGlobal, const std::vector<FileEntry> &morphologyParameters);
    ...
    // Reset methods
    void Reset() override;  // Wrapper plus clearing some of the vectors. Last Reset method to run in chronological order, where we call the
                            // ticks and the general upkeep
    // void DeleteEffects();//Here, if counter==countMax, erase in that index the element of every vector (first store index, then REVERSE
    // remove the removelist indexes with .rbegin and .rend instead of .begin and .end) Container should be ordered by definition, but
    // std::sort(array.begin(), array.end()) would ensure so.
    ...
    bool IgnoreJcoupling() const override { return true; }
};
