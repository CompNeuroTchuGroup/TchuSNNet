...
class NeuronPop {
  ...
  public:
    ...
    double GetTimeSinceLastSpike(NeuronInt neuron) const {
      return static_cast<double>(previousSpikeDistance.at(neuron)) * infoGlobal->dtTimestep;
    }
    ...
    int GetNoSynapses() const {
      return std::accumulate(morphology.begin(), morphology.end(), 0, [](int accumulator, const std::unique_ptr<Morphology>& morpho) {
        return accumulator + morpho->GetNoSynapses();
      });
    ...
    bool ignoreJDistribution() {
      if (morphology.empty()) {
        throw "Logical error jdis";
      } else {
        return morphology.at(0)->IgnoreJDistribution();
      }
    }

    // All of the following functions throw to ease the virtualization, but it is a bad coding practice

    void RecordExcitatorySynapticSpike(NeuronInt neuronId, NeuronInt spineID) {
      this->morphology.at(neuronId)->RecordExcitatoryPreSpike(spineID);
    }
