...
void DictatNeuronPop::ReadInstructionsFromFile() {
  ...
  And ALWAYS time ordered with correct intervals (wrong starttimes will essentially change the first AP firing timestep or total
  phase)->(timeste-starttime)%frequency
  */
...
void DictatNeuronPop::GenerateRegularSpikersFromInstructions() {
  ...
  //         if ((!instruction.completed) && (instruction.startTimeStep>infoGlobal->timeStep)){ //Here not >= to avoid a spike of frequency
...
void DictatNeuronPop::ReadSpikersFromFile() {
  ...
  */
  ...
  while (std::getline(inputStreamFile, line, '\n')) {
    if (line[0] == '>') {
      FileEntry s_entry { SplitStringToEntry(static_cast<std::string>(line)) };
