...
void EIFNeuronPop::Advect(const std::vector<double> &synaptic_dV) {
  ...
  for (NeuronInt neuronID : std::ranges::views::iota(0, noNeurons)) {
    ...
    //(b)Advect
    membraneV.at(neuronID) += dtTimestep / membraneVTau *
                                  (-(membraneV.at(neuronID) - leakPotential) +
                                   dtTimestep / membraneVTau * sharpness * exp((membraneV.at(neuronID) - criticalV) / sharpness)) +
                              synaptic_dV.at(neuronID);
