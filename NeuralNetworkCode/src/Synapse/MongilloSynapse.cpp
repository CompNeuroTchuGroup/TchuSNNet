...
std::vector<double> MongilloSynapse::AdvectSpikers(NeuronInt spiker) {
  ...
  std::replace_if(
      y_Mongillo.at(spiker).begin(), y_Mongillo.at(spiker).end(),
      [this, expTauF](bool state) { return state && (uniformDistribution(generator) < (1.0 - expTauF)); }, false);
  std::replace_if(
      y_Mongillo.at(spiker).begin(), y_Mongillo.at(spiker).end(),
      [this](bool state) { return !state && (uniformDistribution(generator) < u_Mongillo); }, true);
  std::replace_if(
      x_Mongillo.at(spiker).begin(), x_Mongillo.at(spiker).end(),
      [this, expTauD](bool state) { return !state && (uniformDistribution(generator) < (1.0 - expTauD)); }, true);

  // This for loop is PARALLELIZABLE and thread safe, as currents does not change in size and threads do not need to write in the same position ever,
  // but the estimated gains are probably not that good to risk it.
...
std::string MongilloSynapse::GetDataHeader(int dataColumn) {
  return "#" + std::to_string(dataColumn) + " J_" + GetIdStr() + " (mV)\n" + "#" + std::to_string(dataColumn + 1) + " <x>_" + GetIdStr() +
         " ( x is given pre-spike : x = x_postspike + Spikesubmitted ) \n" + "#" + std::to_string(dataColumn + 2) + " <y>_" + GetIdStr() +
         " ( y is given pre-spike : y = (y_postspike - U)/(1-U) ) \n" + "#" + std::to_string(dataColumn + 3) + " <pR>_" + GetIdStr() + "\n";
...
std::vector<double> MongilloSynapse::GetSynapticState(NeuronInt sourceNeuron) const {
  ...
  int xSum{std::accumulate(x_Mongillo.at(sourceNeuron).begin(), x_Mongillo.at(sourceNeuron).end(), 0,
                           [](int accumulator, bool value) { return accumulator + value; })};
  int ySum{std::accumulate(y_Mongillo.at(sourceNeuron).begin(), y_Mongillo.at(sourceNeuron).end(), 0,
                           [](int accumulator, bool value) { return accumulator + value; })};
  // int XY=0;
  int       SpikeSubmitted{std::accumulate(spikeSubmitted.at(sourceNeuron).begin(), spikeSubmitted.at(sourceNeuron).end(), 0,
                                           [](int accumulator, bool value) { return accumulator + value; })};
  ...
  value.at(2) = static_cast<double>((ySum - u_Mongillo * noTargets) /
                                    (1 - u_Mongillo)); // expected value of y before the spike-induced increase in bound calcium probability
