...
PRGSynapseContinuous::PRGSynapseContinuous(PopPtr targetPop, PopPtr sourcePop, GlobalSimInfo *infoGlobal)
    : MongilloSynapseContinuous(targetPop, sourcePop, infoGlobal) {
...
void PRGSynapseContinuous::SaveParameters(std::ofstream &wParameterStream, std::string idString) const {
  ...
  wParameterStream << idString << "prg_Delta_tau_f\t\t\t\t\t" << std::to_string(deltaTauF_PRG)
                   << " #Increase of tauF_Mongillo (as defined in MongilloSynapseContinuous) due to LPA : tauf -> tauF_Mongillo+l*Delta_tau_f\n";
  wParameterStream << idString << "prg_Delta_U\t\t\t\t\t" << std::to_string(deltaU_PRG)
                   << " #Increase of U (as defined in MongilloSynapseContinuous) due to LPA : U -> U+l*Delta_U\n";
...
std::string PRGSynapseContinuous::GetDataHeader(int dataColumn) {
  return "#" + std::to_string(dataColumn) + " J_" + GetIdStr() + " (mV)\n" + "#" + std::to_string(dataColumn + 1) + " <x>_" + GetIdStr() +
         " ( x is given post-spike, as it was afetr last time the presynaptic neuron spiked) \n" + "#" + std::to_string(dataColumn + 2) + " <y>_" +
         GetIdStr() + " ( y is given post-spike) \n" + "#" + std::to_string(dataColumn + 3) + " <l>_" + GetIdStr() + " ( l is given post-spike) \n" +
...
std::vector<double> PRGSynapseContinuous::GetSynapticState(NeuronInt sourceNeuron) const {
  ...
  for (NeuronInt targetNeuron : std::ranges::views::iota(0, noTargetNeurons)) {
    ...
    lMinus = (l_PRG.at(sourceNeuron).at(targetNeuron) -
              M_PRG * y_MongilloC.at(sourceNeuron).at(targetNeuron) * x_MongilloC.at(sourceNeuron).at(targetNeuron)) /
             (1 - M_PRG * y_MongilloC.at(sourceNeuron).at(targetNeuron) * x_MongilloC.at(sourceNeuron).at(targetNeuron));
    sumLMinus += lMinus;
    ...
    if (y_MongilloC.at(sourceNeuron).at(targetNeuron) == 1) {
      ...
      // x_minus = spikeSubmitted.at(sourceNeuron).at(targetNeuron)/(GetCouplingStrength(targetNeuron, sourceNeuron) *
      // y_Mongillo.at(sourceNeuron).at(targetNeuron));
      sumXMinus += spikeSubmitted.at(sourceNeuron).at(targetNeuron) /
                   (GetCouplingStrength(targetNeuron, sourceNeuron) * y_MongilloC.at(sourceNeuron).at(targetNeuron));
    } else {
...
std::vector<double> PRGSynapseContinuous::AdvectSpikers(NeuronInt spiker) {
  double dtLastSpike =
      sourcePop->GetTimeSinceLastSpike(spiker); // static_cast<double>(infoGlobal->timeStep - neuronsPre->get_previous_spike_step(spiker))*dtTimestep;
  ...
  for (NeuronInt targetNeuron : std::ranges::views::iota(0, static_cast<NeuronInt>(currents.size()))) {
    ...
    y_MongilloC.at(spiker).at(targetNeuron) +=
        (u_MongilloC + l_PRG.at(spiker).at(targetNeuron) * deltaU_PRG) * (1 - y_MongilloC.at(spiker).at(targetNeuron));
...
double PRGSynapseContinuous::TransmitSpike(NeuronInt targetNeuron, NeuronInt spiker) {
  ...
  l_PRG.at(spiker).at(targetNeuron) +=
      M_PRG * y_MongilloC.at(spiker).at(targetNeuron) * x_MongilloC.at(spiker).at(targetNeuron) * (1 - l_PRG.at(spiker).at(targetNeuron));
