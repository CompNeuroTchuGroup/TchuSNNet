...
class Synapse {
  protected:
    ...
    std::vector<std::vector<int>>
      DelayDDistribution;  // the list with delays D that are associated with the synapses betweeen the sourceNeuron and its target neurons
    ...
    double GetDistributionJ(NeuronInt targetNeuronIndex, NeuronInt sourceNeuron) const {
      return (HasJDistribution) ? JCouplingDistribution.at(sourceNeuron).at(targetNeuronIndex) : J;
    }
    int GetDistributionD(NeuronInt targetNeuron, NeuronInt sourceNeuron) const {
      return (HasDdistribution) ? DelayDDistribution.at(sourceNeuron).at(targetNeuron) : Dmax;
    }
    // double GetCouplingStrengthTrainablePlasticity(signed long sourceNeuron, signed long targetNeuron); //Move to new class, where virtual
    // function goes Necessary mutex locks std::mutex _waitingMatrixMutexLock;
  ...
  public:
    ...
    double GetRecurrentInput(NeuronInt targetNeuron) const {
      return waitingMatrix.at((this->infoGlobal->timeStep) % (Dmax + 1)).at(targetNeuron);
    }
    ...
    NeuronInt GetNoTargetedSynapses(NeuronInt sourceNeuron) const {
      return static_cast<NeuronInt>(targetSpineList.at(sourceNeuron).size());
    }
    ...
    bool IsTargetLastInVector(NeuronInt targetNeuron, NeuronInt sourceNeuron) const {
      return targetSpineList.at(sourceNeuron).back().first == targetNeuron;
    }
};
