...
std::vector<double> PowerLawSynapse::AdvectSpikers(NeuronInt spiker) {
  ...
  if (noSpikes > 1) {
    averageFrequency =
        (static_cast<double>(noSpikes - 1) / noSpikes) * averageFrequency; // the expected value of 1/ISI is nu*N/(N-1) for a Poisson process
  }
  ...
  for (NeuronInt targetNeuron : std::ranges::views::iota(0, static_cast<NeuronInt>(targetSpineList.at(spiker).size()))) {
    currents.at(targetNeuron) =
        GetCouplingStrength(targetNeuron, spiker) * exponent; // No plus equal because the currents vector is based on indexes, not actual neuron ids
  }
  return currents;
}
