...
MongilloSynapseContinuous::MongilloSynapseContinuous(PopPtr targetPop, PopPtr sourcePop, GlobalSimInfo *infoGlobal)
    : Synapse(targetPop, sourcePop, infoGlobal) {
  // SetSeed(0);
...
void MongilloSynapseContinuous::ConnectNeurons() {
  ...
  for (NeuronInt sourceNeuron : std::ranges::views::iota(0, GetNoSourceNeurons())) {
    ...
    // std::cout << "max x_Mongillo " <<
    // std::to_string(*std::max_element(std::begin(x_Mongillo.at(sourceNeuron)),std::end(x_Mongillo.at(sourceNeuron)))) << "\n"; std::cout << "min
    // x_Mongillo " << std::to_string(*std::min_element(std::begin(x_Mongillo.at(sourceNeuron)),std::end(x_Mongillo.at(sourceNeuron)))) << "\n";
    // std::cout << "\n";
  }
