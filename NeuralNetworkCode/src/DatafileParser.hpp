...
protected:
  ...
  std::vector<std::string> fileNamesToParse;
  // Here we store the file names to open. A similar path will be used for writing, adding _parsed to the filename
  ...
  std::vector<recNeuronPopData> rasterPlotMetadata;
  // Here we store any extra information to write in the file that may be necessary for reading the datafile in the future.
...
public:
  ...
  std::vector<std::vector<std::pair<std::vector<double>, std::pair<PopInt, NeuronInt>>>> parseSpikesToSpikeTimes(std::ifstream &fileStream);
  // Create a vector of vectors (one per neuron) of spike times from the read file
