...
void PoissonConnectivity::SaveParameters(std::ofstream &wParameterStream, std::string idString) const {
  ...
  // }
  wParameterStream << "#\t\t" << IDstringPoissonConnectivity
                   << ": Connectivity between two neurons is determined by a lambda distribution, multiple synapses are enabled.\n";
  // wParameterStream << "#\t\t"<< IDstringBinaryrandomConnectivity << ": Erdoes Renyi network. Each neuronal pair is connected with probability
  // connectionProbability. (as used by [Renart et al. (2010)]). \n";
